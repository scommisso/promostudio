...
namespace PromoStudio.Web
{
    ...
    public static class MvcExtensions
    {
        ...
        /// <returns></returns>
        public static HelperResult RenderSection(this WebPageBase page, string sectionName,
        ...
        {
            string header = intance.Headers["X-PJAX"] ?? string.Empty;

            bool result = header.Equals(
        ...
        public static ActionResult CssFromView(this Controller controller, string cssViewName = null,
            object model = null)
        {
            string cssContent = ParseViewToContent(controller, cssViewName, "style", model);
        ...
        /// "model">optional model to pass to partial view for rendering
        public static ActionResult JavaScriptFromView(this Controller controller, string javascriptViewName = null,
            object model = null)
        {
            string jsContent = ParseViewToContent(controller, javascriptViewName, "script", model);
        ...
        private static string ParseViewToContent(Controller controller, string viewName, string tagName,
            object model = null)
        {
            using (var viewContentWriter = new StringWriter())
            {
                ...
                ViewEngineResult viewEngineResult =
                    controller.ViewEngineCollection.FindPartialView(controller.ControllerContext, viewName);
                ...
                try
                {
                    var viewContext = new ViewContext(controller.ControllerContext, viewEngineResult.View,
                        controller.ViewData, controller.TempData, viewContentWriter);
                    viewEngineResult.View.Render(viewContext, viewContentWriter);
                    string viewString = viewContentWriter.ToString().Trim('\r', '\n', ' ');
                    string regex = string.Format("<{0}[^>]*>(.*?)</{0}>", tagName);
                    Match res = Regex.Match(viewString, regex,
                    ...
                    if (res.Success && res.Groups.Count > 1)
                        ...
                        throw new InvalidProgramException(
                            string.Format(
                                "Dynamic content produced by viewResult '{0}' expected to be wrapped in '{1}' tag",
                                viewName, tagName));
                }
                finally
                {
