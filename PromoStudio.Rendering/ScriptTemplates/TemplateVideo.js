...
for (i = 0; i < swapItems.length; i++) {
	...
	for (j = 1; j <= project.items.length; j++) {
		...
		if (swapItem.type === "Footage") {
			...
			if (item.typeName !== swapItem.type || item.name !== swapItem.comp) { continue; }
			...
			for (k = 1; k < item.layers.length; k++) {
				layer = item.layer(k);
				if (layer.matchName !== "ADBE Text Layer" || layer.name !== swapItem.layer) { continue; }
...
for (i = 1; i <= project.items.length; i++) {
	...
	if (item.typeName !== "Composition") { continue; }
