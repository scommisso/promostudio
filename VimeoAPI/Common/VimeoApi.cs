...
namespace Vimeo.API
{
    ...
    public class AdvancedAPI
    {
        ...
        public bool ExecutePutCommand(string url, string userName,
            string password, Stream data, string contentType, bool statusCheck = false, WebProxy proxy = null)
        {
            ...
            if (!statusCheck)
            {
                ...
                using (Stream tempStream = req.GetRequestStream())
                {
                    do
                    {
            ...
            {
                req.ContentLength = 0;
                req.Headers.Add("Content-Range", "bytes */*");
            }
            ...
            using (var resp = (HttpWebResponse) req.GetResponse())
            {
                if (statusCheck)
                {
                    ...
                    {
                        var rangeHdr = resp.Headers["Range"];
                        var match = Regex.Match(rangeHdr, "bytes=(/d+)-(/d+)", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                        if (match.Groups.Count == 3)
                        {
                            long uploaded = 0;
                            long outOf = 0;
                            if (!long.TryParse(match.Groups[1].Value, out uploaded))
                                return false;
                            if (!long.TryParse(match.Groups[2].Value, out outOf))
                                return false;
                            if (uploaded != outOf || outOf != data.Length)
                                return false;
                            return true;
                        }
                    }
