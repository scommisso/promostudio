...
namespace Vimeo.API
{
    ...
    public partial class VimeoClient
    {
        ...
        public bool ExecutePutRequest(string method, Stream data, string contentType, bool statusCheck = false, Dictionary<string, string> parameters = null)
        {
            return ExecutePutRequest(GetRequestUrl(
                AdvancedAPI.StandardAdvancedApiUrl, method, parameters), data, contentType, statusCheck);
        }

        public bool ExecutePutRequest(string url, Stream data, string contentType, bool statusCheck = false)
        {
            if (url == null)
            {
#if DEBUG && VFW
            ...
            try
            {
                return VimeoAPI.ExecutePutCommand(url, null, null, data, contentType, statusCheck, Proxy);
            }
            catch
            {
        ...
        //.setPreset
        public bool vimeo_videos_embed_setPreset(string video_id, string preset_id)
        {
            Dictionary<string, string> parameters = new Dictionary<string, string>{
                {"video_id", video_id.ToString()},
                {"preset_id", preset_id.ToString()}
            };
            var x = ExecuteGetRequest("vimeo.videos.embed.setPreset", parameters);
            return IsResponseOK(x);
        }
        ...
        public bool vimeo_videos_upload(Ticket ticket, Stream data, string contentType)
        {
            return vimeo_videos_upload(ticket.id, data, contentType);
        }

        public bool vimeo_videos_upload(string ticket_id, Stream data, string contentType)
        {
            Dictionary<string, string> parameters = new Dictionary<string, string> { { "ticket_id", ticket_id } };
            return ExecutePutRequest("vimeo.videos.upload", data, contentType, false, parameters);
        }

        public bool vimeo_videos_upload_isstreamcomplete(Ticket ticket, Stream data, string contentType)
        {
            return vimeo_videos_upload(ticket.id, data, contentType);
        }

        public bool vimeo_videos_upload_isstreamcomplete(string ticket_id, Stream data, string contentType)
        {
            Dictionary<string, string> parameters = new Dictionary<string, string> { { "ticket_id", ticket_id } };
            return ExecutePutRequest("vimeo.videos.upload", data, contentType, true, parameters);
        }
        ...
        //.getTicket
        public Ticket vimeo_videos_upload_getTicket(string video_id=null, string mode=null, string uploadMethod=null)
        {
            Dictionary<string, string> parameters = new Dictionary<string, string>();
            if (!string.IsNullOrEmpty(video_id)) parameters.Add("video_id", video_id);
            if (!string.IsNullOrEmpty(mode)) parameters.Add("mode", mode);
            if (!string.IsNullOrEmpty(uploadMethod)) parameters.Add("upload_method", uploadMethod);
